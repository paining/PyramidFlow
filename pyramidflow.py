...
class SemiInvertible_3x3Conv(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels ) -> None:
        ...
        super().__init__(in_channels, out_channels, kernel_size=3, bias=False, padding=1, padding_mode="replicate")
        nn.init.orthogonal_(self.weight.data) # orth initialization
    def inverse(self, output):
        b, c, h, w = output.shape
        # A = self.weight[..., 0,0] # outch, inch
        A = self.weight.reshape(c, -1)
        B = output.permute([1,0,2,3]).reshape(c, -1) # outch, bhw
        X = torch.linalg.lstsq(A, B)  # AX=B
        sol = X.solution.reshape(-1, b, h, w).permute([1, 0, 2, 3]).flatten(2)
        divisor = F.fold(
            torch.ones_like(sol),
            output_size=sol.shape[-2:],
            kernel_size=3,
            padding=1
        )
        return F.fold(sol, output_size=sol.shape[-2:], kernel_size=3, padding=1) / divisor
    @property
    def logdet(self):
...
class PyramidFlow_CFA(nn.Module):
    ...
    def __init__(self, resnetX, channel, num_level, num_stack, ksize, vn_dims, savemem=False):
        super().__init__()
        assert num_level >= 2
        assert resnetX in [18, 34, 0, "_cfa"]
        # if resnetX==0:
        ...
        self.channel = channel if resnetX==0 else 256 if resnetX=="_cfa" else 64
        ...
        if resnetX != 0:
            if resnetX == "_cfa":
                resnet = wide_resnet50_2(True, True, padding_mode="replicate")
            else:
            ...
            self.inconv = SemiInvertible_3x3Conv(3, self.channel)
